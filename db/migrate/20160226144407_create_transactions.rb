        ...
        execute <<-EOL.strip_heredoc.sql_format(db_adapter)
          ALTER TABLE transactions
            ADD CONSTRAINT only_virtual_transaction_can_have_separate_transaction_uid CHECK (
              ((`transactions`.`type` != 'virtual') AND (`transactions`.`separate_transaction_uid` IS NULL)) OR
              ((`transactions`.`type` = 'virtual') AND (`transactions`.`separate_transaction_uid` IS NOT NULL))
            )
        EOL
        execute <<-EOL.strip_heredoc.sql_format(db_adapter)
          ALTER TABLE transactions
            ADD CONSTRAINT virtual_transaction_can_not_be_seperated CHECK (
              (`transactions`.`type` != 'virtual') OR
              ((`transactions`.`type` = 'virtual') AND (`transactions`.`separated` = 'f'))
            )
        EOL
        execute <<-EOL.strip_heredoc.sql_format(db_adapter)
          ALTER TABLE transactions
            ADD CONSTRAINT on_record_type_and_value_match CHECK (
              (`type` = 'not_on_record' AND `transactions`.`on_record` = 'f') OR
              (`type` != 'not_on_record' AND `transactions`.`on_record` = 't')
            )
        EOL
      end
      dir.down do
