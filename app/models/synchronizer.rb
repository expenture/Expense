    ...
    def register(syncer_class)
      ...
      if syncer_class::PASSCODE_INFO.present?
        ...
        syncer_class::PASSCODE_INFO.each_pair do |k, v|
          ...
          syncer_class.validates "passcode_#{k}", presence: true, if: ->(o) { o.new_record? || o.send("encrypted_passcode_#{k}_changed?") } if v[:required]
          syncer_class.validates "passcode_#{k}", format: v[:format], if: ->(o) { o.new_record? || o.send("encrypted_passcode_#{k}_changed?") } if v[:format].present?
        end
      end
